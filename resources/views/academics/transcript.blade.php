...
@section('title', 'Official Academic Transcript & Semester Grade Sheet')
@section('header_title', 'Student Official Transcript')
...
<div class="space-y-6">
    ...
    <!-- Action Bar & Student Selection Dropdown (Hidden on Print) -->
    <div class="no-print bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <form action="{{ route('academics.transcript') }}" method="GET" class="flex items-center space-x-3 w-full md:w-auto flex-1 max-w-xl">
            <label class="text-xs font-extrabold text-slate-700 uppercase tracking-wider shrink-0">Select Student:</label>
            <select name="student_id" onchange="this.form.submit()" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                @foreach($studentsList as $st)
                    <option value="{{ $st->id }}" {{ $student && $student->id == $st->id ? 'selected' : '' }}>
                        {{ $st->roll_number ?? $st->registration_number }} - {{ $st->full_name }} ({{ $st->program->code ?? 'PROG' }})
                    </option>
                @endforeach
            </select>
        </form>
        ...
        <div class="flex items-center space-x-3 shrink-0">
            <button onclick="window.print()" class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-xs rounded-xl shadow-lg transition flex items-center space-x-2">
                <i class="fa-solid fa-print"></i>
                <span>Print / Save Official Transcript (PDF)</span>
            </button>
        </div>
    </div>
        ...
        <div class="print-container bg-white p-8 md:p-12 rounded-2xl border border-slate-200 shadow-xl max-w-4xl mx-auto space-y-8 text-slate-900">
            ...
            <div class="border-b-2 border-slate-900 pb-6 text-center space-y-2">
                ...
                <p class="text-xs font-bold text-slate-600 tracking-wide uppercase">
                    Office of the Controller of Examinations • Official Semester Grade Sheet
                </p>
            ...
            <div class="space-y-3">
                <h3 class="text-xs font-extrabold text-slate-800 uppercase tracking-wider">Semester Course Grades Evaluation</h3>
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="p-6 text-center text-slate-400 italic">
                                        No evaluation records submitted for this student yet.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            <div class="pt-12 border-t border-slate-200 grid grid-cols-2 gap-8 text-xs text-slate-600">
                <div class="space-y-10">
                    <p class="text-[10px] font-medium text-slate-400 italic">
                        * Errors and omissions exempted. This transcript is electronically verified by UVAS Swat UMS Portal.
                    </p>
    ...
    @else
        <div class="bg-white p-12 rounded-2xl border border-slate-200 text-center text-slate-400 space-y-3">
            ...
            <h3 class="font-bold text-slate-700 text-base">No Student Selected</h3>
            <p class="text-xs">Please select a student from the dropdown above to view their official transcript.</p>
        </div>
    @endif
